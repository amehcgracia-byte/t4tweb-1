...
function applyLegacyTextStyleFields(result: CSSProperties, styles: Record<string, unknown>): CSSProperties {
  ...
  if (styles.gradientEnabled === false) {
    result.backgroundImage = "none"
    result.backgroundClip = "initial"
    result.WebkitBackgroundClip = "initial"
    result.WebkitTextFillColor = typeof result.color === "string" ? result.color : "currentColor"
  }
  return result
}
...
function getHeroLogoStyle(elementStyles: HeroData["elementStyles"]): CSSProperties {
  const rawStyles = getRawElementStyle(elementStyles, "hero-logo")
  const baseStyle = getElementLayoutStyle(elementStyles, "hero-logo", { includeGeometry: true })
...
function getHeroEditorGeometryAttrs(elementStyles: HeroData["elementStyles"], targetId: string): Record<string, string> {
  const styles = getRawElementStyle(elementStyles, targetId)
    ...
    >
        ...
        <div className="flex flex-col items-center pb-6 pt-8 text-center sm:pb-8 sm:pt-12">
          ...
          <div
            ...
            style={{
              ...
              ...getHeroLogoStyle(data.elementStyles),
            }}
          >
            <Image
              src={content.logoUrl}
      ...
      >
        <span
          ...
          {...getHeroEditorGeometryAttrs(data.elementStyles, "hero-scroll-label")}
