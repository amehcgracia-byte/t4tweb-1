...
type Command =
  ...
  | { type: "ADD_NODE"; kind: "text" | "button" | "section"; targetNodeId?: string }
...
function TextToolControls({ node, onPatch, includeContent = true, includeOpacity = true }: TextToolControlsProps) {
  ...
  const selectedFontPreset = EDITOR_FONT_PRESETS.some((preset) => preset.value === node.style.fontFamily)
    ...
    <div className="space-y-2 rounded border border-slate-200 p-2">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => {
              if (e.target.value !== "custom") onPatch({ fontFamily: e.target.value })
            }}
          >
            {EDITOR_FONT_PRESETS.map((preset) => <option key={preset.value} value={preset.value}>{preset.label}</option>)}
      ...
      <div className="grid grid-cols-2 gap-2">
        <label className="text-[10px]">
          ...
          <select
            ...
            onChange={(e) => onPatch({ textTransform: e.target.value as EditorNode["style"]["textTransform"] })}
          >
        ...
        <label className="text-[10px]">
          ...
          <input
            className="mt-1 w-full rounded border p-1 text-xs"
            value={node.style.textShadow || ""}
            onChange={(e) => onPatch({ textShadow: e.target.value })}
            placeholder="1px 2px 4px #000"
          />
        </label>
      </div>
      <div>
      ...
      <div className="grid grid-cols-2 gap-2 border-t border-slate-100 pt-2">
        <label className="text-[10px]">
          Border color
          <EditorColorInput
            className="mt-1 h-8 w-full rounded border p-1"
            value={node.style.borderColor}
            fallback="#ffffff"
            onValueChange={(value) => onPatch({ borderColor: value })}
          />
        </label>
        <label className="text-[10px]">
          Border width
          <input className="mt-1 w-full rounded border p-1 text-xs" value={node.style.borderWidth || ""} onChange={(e) => onPatch({ borderWidth: e.target.value })} placeholder="1px" />
        </label>
        <label className="text-[10px]">
          Border radius
          <input className="mt-1 w-full rounded border p-1 text-xs" value={node.style.borderRadius || ""} onChange={(e) => onPatch({ borderRadius: e.target.value })} placeholder="8px" />
        </label>
        <label className="text-[10px]">
          Box shadow
          <input className="mt-1 w-full rounded border p-1 text-xs" value={node.style.boxShadow || ""} onChange={(e) => onPatch({ boxShadow: e.target.value })} placeholder="0 4px 12px #0008" />
        </label>
        <label className="text-[10px]">
          Padding left
          <input className="mt-1 w-full rounded border p-1 text-xs" value={node.style.paddingLeft || ""} onChange={(e) => onPatch({ paddingLeft: e.target.value })} placeholder="1rem" />
        </label>
        <label className="text-[10px]">
          Padding right
          <input className="mt-1 w-full rounded border p-1 text-xs" value={node.style.paddingRight || ""} onChange={(e) => onPatch({ paddingRight: e.target.value })} placeholder="1rem" />
        </label>
      </div>
    </div>
  )
...
function DecorationToolControls({ node, onPatch, label }: { node: EditorNode; onPatch: (patch: TextEditorPatch) => void; label: string }) {
  return (
    <div className="grid grid-cols-2 gap-2 rounded border border-slate-200 p-2">
      <div className="col-span-2 text-[10px] font-semibold">{label}</div>
      <label className="text-[10px]">Border color<EditorColorInput className="mt-1 h-8 w-full rounded border p-1" value={node.style.borderColor} fallback="#ffffff" onValueChange={(value) => onPatch({ borderColor: value })} /></label>
      <label className="text-[10px]">Border width<input className="mt-1 w-full rounded border p-1 text-xs" value={node.style.borderWidth || ""} onChange={(e) => onPatch({ borderWidth: e.target.value })} placeholder="1px" /></label>
      <label className="text-[10px]">Radius<input className="mt-1 w-full rounded border p-1 text-xs" value={node.style.borderRadius || ""} onChange={(e) => onPatch({ borderRadius: e.target.value })} placeholder="8px" /></label>
      <label className="text-[10px]">Box shadow<input className="mt-1 w-full rounded border p-1 text-xs" value={node.style.boxShadow || ""} onChange={(e) => onPatch({ boxShadow: e.target.value })} placeholder="0 4px 12px #0008" /></label>
      <label className="text-[10px]">Padding left<input className="mt-1 w-full rounded border p-1 text-xs" value={node.style.paddingLeft || ""} onChange={(e) => onPatch({ paddingLeft: e.target.value })} placeholder="1rem" /></label>
      <label className="text-[10px]">Padding right<input className="mt-1 w-full rounded border p-1 text-xs" value={node.style.paddingRight || ""} onChange={(e) => onPatch({ paddingRight: e.target.value })} placeholder="1rem" /></label>
    </div>
  )
}
...
function buildNodeFromEntry(entry: RuntimeEntry): EditorNode {
  ...
  const baseNode: EditorNode = {
    ...
    style: {
      ...
      textTransform: cs.textTransform as EditorNode["style"]["textTransform"],
      ...
      objectFit: cs.objectFit as EditorNode["style"]["objectFit"],
...
export function VisualEditorProvider({ children }: { children: ReactNode }) {
  ...
  // the next React pass. Re-scan then so they immediately become selectable,
  // movable, and persistable just like built-in nodes.
  useEffect(() => {
    if (!isEditing) return
    const timer = window.setTimeout(() => setRegistry(scanRegistry()), 0)
    return () => window.clearTimeout(timer)
  }, [isEditing, nodes])
  ...
  const applyNodeToDom = useCallback((node: EditorNode, entry: RuntimeEntry) => {
    ...
    } else if (!isViewportContainer && !isCustomNode && (!isMediaNode(node) || node.explicitContent) && (node.explicitPosition || (node.explicitStyle && nodeScale !== 1))) {
    ...
    if (!isViewportContainer && !isCustomNode && (!isMediaNode(node) || node.explicitContent) && node.explicitSize) {
    ...
    if (node.type === "image" || node.type === "background") {
      ...
      if (node.explicitStyle) {
        const media = img || iframe || el
        if (node.style.objectFit) media.style.objectFit = node.style.objectFit
        if (node.style.objectPosition) media.style.objectPosition = node.style.objectPosition
      }
    }
  ...
  const dispatch = useCallback((command: Command) => {
    ...
    setNodes((prev) => {
      ...
      switch (command.type) {
        ...
        case "UPDATE_SECTION": {
          patchNode(command.nodeId, (n) => {
            ...
            Object.entries(command.patch).forEach(([k, v]) => {
              if (["text", "textSegments", "titleSegments", "href", "src", "alt", "videoUrl", "mediaKind", "gradientEnabled", "gradientStart", "gradientEnd", "customKind"].includes(k)) {
                isContentEdit = true;
        ...
        case "ADD_NODE": {
          const id = `custom-${command.kind}-${Date.now()}`
          const index = Array.from(next.keys()).filter((nodeId) => nodeId.startsWith("custom-")).length
          const target = command.targetNodeId ? next.get(command.targetNodeId) : undefined
          const node: EditorNode = {
            id,
            type: command.kind,
            sectionId: "editor-custom-canvas",
            label: command.kind === "text" ? "New text" : command.kind === "button" ? "New button" : "New section",
            isGrouped: false,
            geometry: {
              x: target ? Math.max(16, target.geometry.x) : 24,
              y: target ? target.geometry.y + target.geometry.height + 24 : 24 + index * 86,
              width: command.kind === "section" ? 920 : command.kind === "button" ? 220 : 320,
              height: command.kind === "section" ? 120 : command.kind === "button" ? 48 : 56,
            },
            style: {
              color: "#ffffff",
              backgroundColor: command.kind === "button" ? "#FF8C21" : "transparent",
              fontFamily: "var(--font-inter)",
              fontSize: command.kind === "section" ? "20px" : "16px",
              borderRadius: command.kind === "button" ? "999px" : "8px",
              paddingLeft: "16px",
              paddingRight: "16px",
            },
            content: {
              text: command.kind === "text" ? "New text" : command.kind === "button" ? "New button" : "New section",
              href: command.kind === "button" ? "#" : undefined,
  ...
  const value: VisualEditorContextType = {
    ...
    getDeletedNodeIds: () => Array.from(deletedIdsRef.current),
    clearDeletedNodeIds: () => deletedIdsRef.current.clear(),
    undo,
    redo,
...
export function VisualEditorOverlay() {
  const { isEditing, setIsEditing, selectedId, nodes, registry, dispatch, getDirtyNodeIds, clearDirtyNodeIds, getDeletedNodeIds, clearDeletedNodeIds, openPanel, setOpenPanel, undo, redo, canUndo, canRedo, assets, getEditableAtPosition } = useVisualEditor()
  ...
  const onDeploy = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        deletedNodeIds: getDeletedNodeIds(),
        ...
        return (
          ...
          >
            ...
            <div className="max-h-[min(70vh,390px)] space-y-1 overflow-y-auto p-2 text-xs">
              ...
              <div className="mt-2 border-t border-slate-100 pt-2">
                <div className="px-2 pb-1 text-[10px] font-semibold uppercase tracking-wide text-slate-400">Añadir contenido publicable</div>
                <div className="grid grid-cols-3 gap-1">
                  {(["text", "button", "section"] as const).map((kind) => (
                    <button
                      key={kind}
                      type="button"
                      className="rounded border border-orange-200 px-2 py-1.5 text-[10px] font-medium text-orange-800 hover:bg-orange-50"
                      onClick={() => { dispatch({ type: "ADD_NODE", kind, targetNodeId: contextMenu.nodeId }); setContextMenu(null) }}
                    >
                      {kind === "text" ? "Texto" : kind === "button" ? "Botón" : "Sección"}
                    </button>
                  ))}
                </div>
                <div className="px-2 pt-1 text-[10px] leading-relaxed text-slate-500">Se crean en un lienzo seguro al final de la página y se guardan con Deploy.</div>
              </div>
            </div>
          </div>
        )
          ...
          <div className="max-h-[calc(100vh-8rem)] space-y-2 overflow-y-auto overscroll-contain p-3 text-slate-900">
            ...
            {(selectedNode.type === "image" || (selectedNode.type === "background" && selectedNode.content.mediaKind !== "video")) && (
              <>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <label className="text-[10px]">
                    Object fit
                    <select className="mt-1 w-full rounded border p-1 text-xs" value={selectedNode.style.objectFit || "cover"} onChange={(e) => dispatch({ type: selectedNode.type === "image" ? "UPDATE_IMAGE" : "UPDATE_BACKGROUND", nodeId: selectedNode.id, patch: { objectFit: e.target.value as "cover" | "contain" | "fill" } })}>
                  ...
                  <label className="text-[10px]">
                    Object position
                    <input className="mt-1 w-full rounded border p-1 text-xs" value={selectedNode.style.objectPosition || "center center"} onChange={(e) => dispatch({ type: selectedNode.type === "image" ? "UPDATE_IMAGE" : "UPDATE_BACKGROUND", nodeId: selectedNode.id, patch: { objectPosition: e.target.value } })} placeholder="center 40%" />
                  </label>
                </div>
              </>
            )}
            ...
            {selectedNode.type === "background" && selectedNode.content.mediaKind === "video" && (
              <>
                <div>
                  <label className="text-[10px]">Contrast ({Math.round(selectedNode.style.contrast ?? 100)}%)</label>
                  <input type="range" min={0} max={200} step={1} className="w-full" value={selectedNode.style.contrast ?? 100} onChange={(e) => dispatch({ type: "UPDATE_BACKGROUND", nodeId: selectedNode.id, patch: { contrast: Number(e.target.value) } })} />
                </div>
                <label className="text-[10px]">Opacity ({(selectedNode.style.opacity ?? 1).toFixed(2)})</label>
                <input
                  type="range"
                ...
                />
                <div className="grid grid-cols-2 gap-2">
                  <label className="text-[10px]">Saturation<input type="range" min={0} max={200} step={1} className="w-full" value={selectedNode.style.saturation ?? 100} onChange={(e) => dispatch({ type: "UPDATE_BACKGROUND", nodeId: selectedNode.id, patch: { saturation: Number(e.target.value) } })} /></label>
                  <label className="text-[10px]">Brightness<input type="range" min={0} max={200} step={1} className="w-full" value={selectedNode.style.brightness ?? 100} onChange={(e) => dispatch({ type: "UPDATE_BACKGROUND", nodeId: selectedNode.id, patch: { brightness: Number(e.target.value) } })} /></label>
                </div>
                <label className="flex items-center gap-2 text-xs"><input type="checkbox" checked={selectedNode.style.negative ?? false} onChange={(e) => dispatch({ type: "UPDATE_BACKGROUND", nodeId: selectedNode.id, patch: { negative: e.target.checked } })} />Negativo</label>
                <label className="text-[10px]">Object position<input className="mt-1 w-full rounded border p-1 text-xs" value={selectedNode.style.objectPosition || "center center"} onChange={(e) => dispatch({ type: "UPDATE_BACKGROUND", nodeId: selectedNode.id, patch: { objectPosition: e.target.value } })} placeholder="center 40%" /></label>
              </>
            )}
            ...
            {selectedNode.type === "section" && (
              <>
                ...
                />
                <DecorationToolControls node={selectedNode} label="Section decoration" onPatch={(patch) => dispatch({ type: "UPDATE_SECTION", nodeId: selectedNode.id, patch })} />
              </>
            )}
