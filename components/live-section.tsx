...
function getConcertLocationDisplay(concert: LiveConcert): string {
  const address = concert.address.trim()
  if (address) return address
  const venueLike = (concert.locationName || concert.venue || "").trim()
  if (venueLike) return venueLike
  return [concert.city, concert.country].filter(Boolean).join(", ") || "Location TBA"
}

function getConcertLocationSuffix(concert: LiveConcert): string {
  if (concert.address.trim()) return ""
  const venueLike = (concert.locationName || concert.venue || "").trim()
  if (venueLike) return ""
  return concert.city && concert.country ? concert.country : ""
}
              ...
              >
                ...
                <div className="mb-5 flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-2 text-sm text-white/70">{getConcertLocationDisplay(activeConcert)}</p>
                  </div>
                </div>
                <dl className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between gap-4">
                    ...
                    <dd className="text-right font-medium">{getConcertLocationDisplay(activeConcert)}</dd>
                  </div>
                  <div className="flex justify-between gap-4">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="text-sm text-white/55">
                              <span
                                ...
                                data-editor-node-type="text"
                              data-editor-node-label={`Upcoming Event ${index + 1} Location Display`}
                                data-concert-field="city"
                                style={getElementLayoutStyle(data.elementStyles, `${cardId}-city`)}
                              >
                                {getConcertLocationDisplay(concert)}
                              </span>
                              {getConcertLocationSuffix(concert) ? " · " : ""}
                              <span
                                data-editor-node-id={`${cardId}-country`}
                                data-editor-node-type="text"
                                data-editor-node-label={`Upcoming Event ${index + 1} Location Suffix`}
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="text-sm text-white/45">
                              <span
                                data-editor-node-id={`${cardId}-city`}
                                data-editor-node-type="text"
                              data-editor-node-label={`History Event ${index + 1} Location Display`}
                                data-concert-field="city"
                                style={getElementLayoutStyle(data.elementStyles, `${cardId}-city`)}
                              >
                                {getConcertLocationDisplay(concert)}
                              ...
                              <span
                                data-editor-node-id={`${cardId}-country`}
                                data-editor-node-type="text"
                                data-editor-node-label={`History Event ${index + 1} Location Suffix`}
                                data-concert-field="country"
                                style={getElementLayoutStyle(data.elementStyles, `${cardId}-country`)}
                              >
                                {getConcertLocationSuffix(concert)}
