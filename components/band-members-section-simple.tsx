...
import type { BandMembersLoadResult } from "@/lib/sanity/band-members-loader"
...
export function BandMembersSectionSimple({ data }: { data: BandMembersLoadResult }) {
  ...
  useEffect(() => {
    const onEditorBandMemberFocus = (event: Event) => {
      ...
      if (index < 0 || index >= data.members.length) return
  ...
  const activeMember = data.members[activeIndex]
          ...
          <div className="space-y-2.5 md:space-y-4">
            {data.members.map((member, index) => (
              <motion.div
                ...
                style={getElementLayoutStyle(data.elementStyles || {}, `member-item-${index}`)}
              >
