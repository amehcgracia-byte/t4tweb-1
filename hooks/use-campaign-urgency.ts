...
function buildUrgencyFromCsv(csv: string): string | null {
  ...
  const futureShows = rows
    .map((row) => ({
      city: row[cityIndex] || "",
      date: row[dateIndex] || "",
      parsedDate: parseDateOnly(row[dateIndex] || ""),
    }))
    .filter((show) => show.date && show.parsedDate && show.parsedDate.getTime() >= startOfToday)
    .sort((a, b) => (a.parsedDate?.getTime() || 0) - (b.parsedDate?.getTime() || 0))

  if (!futureShows.length) return null

  const nextShow = futureShows[0]
  const formattedDate = nextShow.parsedDate?.toLocaleDateString("en-GB", {
    day: "2-digit",
    month: "short",
    year: "numeric",
  })
